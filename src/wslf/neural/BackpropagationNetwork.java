...
public class BackpropagationNetwork extends Network {
     ...
     * @param layers
     */
    public BackpropagationNetwork(Layer[] layers) {
        // передадим родакам
        super(layers);
        // рандомизируем веса
        randomize(0, 0.3f);
    ...
    public void randomize(float min, float max) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (layer instanceof BackpropagationLayer) ((BackpropagationLayer) layer).randomize(min, max);
        }
    }
    ...
    public float learnPattern(float[] input, float[] goal, float rate, float momentum) {
        ...
        if (layer instanceof BackpropagationLayer)
            ((BackpropagationLayer) layer).adjust(size == 1 ? input : outputs[size - 2], error, rate, momentum);
        ...
        for (int i = size - 2; i >= 0; i--, prevError = error, prevLayer = layer) {
            ...
            if (prevLayer instanceof BackpropagationLayer)
                error = ((BackpropagationLayer) prevLayer).computeBackwardError(outputs[i], prevError);
            else
                error = prevError;
            // обновим слой
            if (layer instanceof BackpropagationLayer)
                ((BackpropagationLayer) layer).adjust(i == 0 ? input : outputs[i - 1], error, rate, momentum);
        }
